    ...
    with Session(engine) as session:
        ...
        if provider_uuid:
            ...
            provider = session.exec(
                select(ProviderDB).where(ProviderDB.is_available == True)
            ).first()
        ...
        print(
            f"Creating orders for provider: {provider.business_name} (ID: {provider.id})"
        )
        ...
        for i, status in enumerate(statuses):
            ...
            booking = BookingDB(
                ...
                else None,  # Assign provider unless pending
            ...
            if status == BookingStatus.PENDING:
                ...
                assignment = AssignmentQueueDB(
                    ...
                    + timedelta(hours=24),  # Long expiry so you can see it
                    created_at=datetime.utcnow(),
                )
                session.add(assignment)
                created_assignments.append(assignment.id)
                print(
                    f"  [+] Created PENDING Assignment for Booking {booking.booking_number}"
                )

            session.commit()
            print(
                f"  [+] Created Booking {booking.booking_number} with status {status.value}"
            )
