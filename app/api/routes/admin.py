...
@router.get("/stats", dependencies=[Depends(get_current_active_superuser)], response_model=StatsPublic)
def get_stats(session: SessionDep) -> Any:
    ...
    active_providers = session.exec(select(func.count(ProviderDB.id)).where(ProviderDB.is_available == True)).one()
    
    # Pending booking requests (status='pending' or 'awaiting_provider')
    pending_requests = session.exec(
        select(func.count(BookingDB.id)).where(BookingDB.status.in_(["pending", "awaiting_provider"]))
    ).one()
    ...
    ongoing_bookings = session.exec(
        select(func.count(BookingDB.id)).where(BookingDB.status.in_(["confirmed", "in_progress"]))
    ).one()
