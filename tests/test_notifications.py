...
def test_device_registration_update_existing(session: Session):
    ...
    device = UserDeviceDB(
        user_id=user1.id, fcm_token="existing-token", platform="android"
    )
    session.add(device)
    session.commit()
    ...
    assert (
        updated_device.platform == "android"
    )  # Should preserve platform unless updated? Logic says no update on platform currently
